...
Note that `geom_smooth` needs to know what kind of method we are using and the formula for the calculation of the slope. In this case we are using similar syntax to that of base `R`'s regression model function, `lm`. Our formula simply tells `ggplot()` we want to see the relationship denoted by $y = f(x)$. What if we instead wanted a horizontal line in the middle of the scatterplot? Note that this type of geom requires that you know the coordinates of the data you are plotting. 
