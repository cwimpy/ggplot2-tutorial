...
Now, where things get sometimes confusing, and this is jumping ahead, is when folks add additinal layers to the plot. If we wanted to see some points on the plot with our initial code above we could do this:
...
Notice that we do this by adding a `+` (telling `ggplot()` there is a new layer incoming) and then we type the new code. I like to put each new layer on its own line and indent a little after the first line. That helps human readibility and allows us to root out bugs in the code. Using the object method we could do something similar. 
...
In this case, we have asked `ggplot()` to add points to the plot and thus we now have a scatterplot. The "geoms" are numerous, and they are the key to what type of plot you end up with whether it be a bar graph, line graph, histogram, or more. 
